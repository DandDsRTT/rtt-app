...
const changeMapping = async () => {
    ...
    fireEvent.change(screen.getByTestId("mapping-cell-row-1-column-2"), {target: {value: '5'}})
...
const changeCommaBasis = async () => {
    ...
    fireEvent.change(screen.getByTestId("comma-basis-cell-column-0-row-1"), {target: {value: '-5'}})
    await waitFor(() => expect(screen.queryByText('loading...')).not.toBeTruthy());
}
